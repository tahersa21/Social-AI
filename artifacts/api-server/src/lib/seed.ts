import { db, aiConfigTable, aiProvidersTable, productsTable, fbSettingsTable, adminUsersTable, faqsTable, availableSlotsTable, subscriptionPlansTable, subscriptionUsageTable, domainTemplatesTable, leadsTable, broadcastTemplatesTable } from "@workspace/db";
...
export async function runSeed(): Promise<void> {
  try {
    ...
    if ((adminCount?.value ?? 0) === 0) {
      ...
      console.warn("[seed] WARNING: Default admin created (admin / admin123) — change password immediately!");
    }
