...
router.put("/ai-config", upload.single("pageLogo"), async (req, res): Promise<void> => {
  ...
  const [updated] = await db
    ...
    .set({
      ...
      salesBoostEnabled: body.salesBoostEnabled !== undefined ? Number(body.salesBoostEnabled) : (config as any).salesBoostEnabled,
      salesBoostLevel: body.salesBoostLevel !== undefined ? body.salesBoostLevel : (config as any).salesBoostLevel,
      priceLockEnabled: body.priceLockEnabled !== undefined ? Number(body.priceLockEnabled) : (config as any).priceLockEnabled,
      humanGuaranteeEnabled: body.humanGuaranteeEnabled !== undefined ? Number(body.humanGuaranteeEnabled) : (config as any).humanGuaranteeEnabled,
      smartEscalationEnabled: body.smartEscalationEnabled !== undefined ? Number(body.smartEscalationEnabled) : (config as any).smartEscalationEnabled,
      updatedAt: new Date(),
    })
