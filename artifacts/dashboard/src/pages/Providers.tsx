...
const PROVIDER_TYPES = ["Anthropic", "OpenAI", "Google Gemini", "DeepSeek", "Groq", "OpenRouter", "Orbit", "AgentRouter", "Custom", "raw_single", "raw_messages"];
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-5xl mx-auto">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1,2].map(i => <div key={i} className="h-48 rounded-xl bg-slate-100 animate-pulse" />)}
        </div>
      ) : (
          ...
          {providers.map(provider => {
            ...
            <Card key={provider.id} className={`border border-border/50 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden ${provider.isActive ? 'ring-2 ring-primary border-transparent' : ''} ${provider.isEnabled === 0 ? 'opacity-60' : ''}`}>
              ...
              <CardContent className="space-y-3 pb-4">
                <div className="bg-slate-50 rounded-lg p-3 space-y-2 text-sm border border-border/50">
                  <div className="flex justify-between items-center">
                    <span className="text-muted-foreground">Model</span>
                    <span className="font-mono bg-white px-2 py-0.5 rounded shadow-sm border border-slate-100 text-xs">{provider.modelName}</span>
                  </div>
                  {provider.baseUrl && (
                    <div className="flex justify-between items-center">
              ...
              <CardFooter className="flex flex-wrap gap-2 border-t border-border/50 bg-slate-50/50 pt-4">
                ...
                {provider.isActive === 0 && (
                  <Button variant="outline" size="sm" className="flex-1 min-w-[80px] bg-white hover:text-primary hover:bg-primary/5" onClick={() => {
                ...
                <Button variant="outline" size="sm" className="flex-1 min-w-[80px] bg-white hover:text-amber-600 hover:bg-amber-50" onClick={() => {
      ...
      <Dialog open={curlDialogOpen} onOpenChange={setCurlDialogOpen}>
        <DialogContent dir="rtl" className="max-w-lg">
          ...
          <div className="space-y-4 pt-2">
            <div className="bg-slate-50 rounded-lg p-3 border text-xs font-mono text-slate-600 leading-relaxed">
              <p className="text-slate-500 text-xs mb-1.5 font-sans">مثال:</p>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                  ...
                  <FormLabel className="flex justify-between items-center">
                    ...
                    {!editingProvider && keyUrl && (
                       <a href={keyUrl} target="_blank" rel="noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1">
                         الحصول على مفتاح <LinkIcon className="w-3 h-3" />
                       </a>
                    )}
                  </FormLabel>
                  <FormControl>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {defaults.suggestedModels.map((m) => (
                        <button
                          ...
                          className={`text-xs px-2 py-1 rounded-md border transition-colors ${field.value === m ? "bg-primary text-primary-foreground border-primary" : "bg-slate-50 hover:bg-slate-100 border-border/60 text-slate-700"}`}
                        >
                          {m}
                        </button>
                      ))}
              ...
              <FormField control={form.control} name="baseUrl" render={({ field }) => (
                <FormItem>
                  <FormLabel>الرابط الأساسي / Base URL (اختياري)</FormLabel>
                  <FormControl><Input placeholder="https://api..." {...field} className="h-11 direction-ltr font-mono text-sm" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
