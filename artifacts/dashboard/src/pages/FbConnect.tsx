...
import { Facebook, Save, Loader2, TestTube2, CheckCircle2, XCircle, Copy, Check, Shield, Link, Hash, KeyRound, Webhook, AlertTriangle, LockKeyhole, ExternalLink, Clock, ListChecks } from "lucide-react";
import { useGetFbSettings, useSaveFbSettings, useTestFbConnection } from "@workspace/api-client-react";
...
export default function FbConnect() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-3xl mx-auto">
      ...
      <Card className="border-none shadow-md shadow-slate-200/50 rounded-2xl bg-gradient-to-br from-emerald-50/50 to-teal-50/30">
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              ...
              <div key={perm} className="flex items-start gap-2.5 bg-white rounded-xl border border-emerald-100 px-3 py-2.5 shadow-sm">
                <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                <div>
                  <code className="text-xs font-mono font-bold text-slate-700">{perm}</code>
      ...
      {/* Webhook URL Card */}
      <Card className="border-none shadow-md shadow-slate-200/50 rounded-2xl bg-gradient-to-br from-blue-50/50 to-indigo-50/30">
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2 bg-white rounded-xl border border-border/50 px-4 py-3 shadow-sm">
              <code className="flex-1 text-sm font-mono text-slate-700 break-all">{webhookUrl}</code>
      ...
      {/* Credentials Form */}
      <Card className="border-none shadow-md shadow-slate-200/50 rounded-2xl">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="123456789012345" {...field} className="rounded-xl bg-slate-50/50 border-border/60 focus:bg-white transition-colors" dir="ltr" />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="أي نص سري من اختيارك" {...field} className="rounded-xl bg-slate-50/50 border-border/60 focus:bg-white transition-colors" dir="ltr" />
                      </FormControl>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          {...field}
                          className="rounded-xl bg-slate-50/50 border-border/60 focus:bg-white transition-colors font-mono"
                          dir="ltr"
                        />
                      </FormControl>
                      <FormDescription>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <Button
                    ...
                    className="flex-1 rounded-xl shadow-sm h-11 bg-white border-border/60 hover:bg-slate-50"
                  >
                    {testMut.isFetching ? (
                      <Loader2 className="w-4 h-4 me-2 animate-spin" />
                    ) : (
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {["messages", "messaging_postbacks", "feed", "message_reactions"].map((field) => (
              <Badge key={field} variant="secondary" className="font-mono text-xs px-3 py-1.5 bg-slate-100 text-slate-700">
                {field}
              </Badge>
            ))}
          </div>
