...
import { ShoppingCart, ExternalLink, MessageCircle, MessageSquare, Phone, MapPin, Trash2 } from "lucide-react";
          ...
          <table className="w-full text-sm text-right">
            <thead className="text-xs text-muted-foreground bg-slate-50 border-b border-border/50 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-bold text-slate-500">رقم #</th>
                <th className="px-6 py-4 font-bold text-slate-500">المصدر</th>
                <th className="px-6 py-4 font-bold text-slate-500">العميل</th>
                <th className="px-6 py-4 font-bold text-slate-500">المنتج</th>
                <th className="px-6 py-4 font-bold text-slate-500">الهاتف</th>
                <th className="px-6 py-4 font-bold text-slate-500">الولاية</th>
                <th className="px-6 py-4 font-bold text-slate-500">العنوان</th>
                <th className="px-6 py-4 font-bold text-slate-500">الكمية</th>
                ...
                <th className="px-4 py-4 font-bold text-slate-500"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50 bg-white">
              {isLoading ? (
                <tr>
                  <td colSpan={13} className="p-8 text-center text-muted-foreground animate-pulse">جاري التحميل...</td>
                </tr>
              ) : orders.length === 0 ? (
                <tr>
                ...
                orders.map(order => (
                  <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-mono font-medium text-slate-500">#{order.id}</td>
                    ...
                    <td className="px-6 py-4">
                      <div>
                        ...
                        {order.customerName && order.fbUserName && order.customerName !== order.fbUserName && (
                          <span className="text-xs text-slate-400">FB: {order.fbUserName}</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      {order.customerPhone ? (
                        <span className="flex items-center gap-1 text-xs"><Phone className="w-3 h-3 text-slate-400" />{order.customerPhone}</span>
                      ) : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-6 py-4">
                      {order.customerWilaya ? (
                        <span className="flex items-center gap-1 text-xs"><MapPin className="w-3 h-3 text-slate-400" />{order.customerWilaya}</span>
                      ) : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-6 py-4 text-xs max-w-[150px] truncate" title={order.customerAddress ?? ""}>
                      {order.customerAddress || <span className="text-slate-300">—</span>}
                    </td>
