...
import { Calendar, Clock, Plus, Trash2, Loader2, ExternalLink } from "lucide-react";
import { useListAppointments, useUpdateAppointment, useDeleteAppointment, useListSlots, useCreateSlot, useUpdateSlot, useDeleteSlot } from "@workspace/api-client-react";
...
export default function Appointments() {
  ...
  const handleAddSlot = () => {
    createSlot({ data: { dayOfWeek: parseInt(newSlotDay), timeSlot: newSlotTime, maxBookings: parseInt(newSlotMax) } }, {
      onSuccess: () => {
      ...
      <Tabs defaultValue="bookings" className="w-full">
        ...
        <TabsContent value="bookings" className="space-y-4 mt-4">
          <div className="flex gap-3 flex-wrap">
            <Input type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="w-48 h-10 bg-white" placeholder="تصفية بالتاريخ" />
            <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v === "all" ? "" : v)}>
              <SelectTrigger className="w-48 h-10 bg-white"><SelectValue placeholder="كل الحالات" /></SelectTrigger>
              <SelectContent>
          ...
          {loadingSlots ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {DAY_NAMES.map((dayName, dayIdx) => {
                ...
                return (
                  <Card key={dayIdx} className="border-none shadow-md shadow-slate-200/50">
                    ...
                    <CardContent className="space-y-2">
                      {daySlots.length === 0 ? (
                        ...
                        daySlots.map(slot => (
                          <div key={slot.id} className={`flex items-center justify-between p-2 rounded-lg border ${slot.isActive ? 'bg-green-50/50 border-green-200' : 'bg-slate-50 border-slate-200 opacity-60'}`}>
